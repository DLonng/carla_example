import argparse
'''
json.dumps(): This function serializes Python objects into JSON strings.
json.loads(): This function deserializes JSON strings into Python objects.
json.dump(): This function writes Python objects as JSON to a file.
json.load(): This function reads JSON data from a file and returns Python objects.
'''
import json
import os
...
import torch
'''
distributed training and communication between processes in a distributed system. 
It enables efficient data parallel training, 
where models are trained simultaneously across multiple GPUs or multiple machines.
'''
...
'''
 It provides a high-level interface to log data and visualize it in TensorBoard, 
 a visualization tool for deep learning experiments.
'''
...
It offers a more convenient and readable way to manipulate file 
and directory paths compared to using plain strings
'''
import pathlib
import datetime
'''
This function is used for recording errors 
in the elastic distributed training framework of PyTorch.
'''
from torch.distributed.elastic.multiprocessing.errors import record
import random

'''
This optimizer class is designed for distributed training scenarios 
and reduces communication overhead by eliminating redundant gradient computation.
'''
from torch.distributed.optim import ZeroRedundancyOptimizer

'''
functionality for spawning processes
parallel execution of code on multiple CPU cores or even multiple machines.
'''
import torch.multiprocessing as mp

'''
provides a persistent disk-backed cache that can be used to store and retrieve Python objects. 
It is a useful tool for caching computationally expensive or IO-intensive operations.
'''
...
def main():
    ...
    # /logs, experiment1 -> /logs/experiment1
    ...
    if(bool(args.use_disk_cache) == True):
        if (parallel == True):
            ...
            # tmp_folder: specifies the directory where the cache will be stored
            # size_limit: sets the size limit for the cache in bytes.
            '''
            您可以创建一个可以在磁盘上存储大量数据的持久缓存。
            它提供对存储对象的高效访问，并且可以在代码或进程的不同部分之间共享，
            从而允许共享状态或数据存储。
            '''
    ...
    if(parallel == True): # Non distributed works better with my local debugger
        # 表示分布式训练中所有进程中当前进程的排名。
        rank       = int(os.environ["RANK"])       # Rank accross all processes
        # represents the rank of the current process on the local node.
        ...
        # initializes the distributed training process group
        torch.distributed.init_process_group(backend='nccl', init_method='env://', world_size=world_size, rank=rank,
                                             timeout=datetime.timedelta(minutes=15))
        # 该行使用屏障操作同步分布式训练组中的所有进程。它确保所有流程在继续进行之前都达到这一点。
    ...
    '''
        used to enable cuDNN benchmark mode in PyTorch for optimizing convolutional operations on supported GPUs.
        If the input size or model architecture changes dynamically, it is recommended to 
        set torch.backends.cudnn.benchmark = False to ensure accurate and reproducible results.
    '''
    ...
    if(bool(args.no_bev_loss)):
        # It assumes that "loss_bev" is present in the list 
        # and assigns the index to the variable index_bev.
        index_bev = config.detailed_losses.index("loss_bev")
        '''
            通过将特定损失的权重设置为 0.0，您可以在训练或评估期间从整体损失计算中排除该特定损失。
            当您想要有选择地忽略或禁用模型中的某些损失时，这会很有用。
        '''
        config.detailed_losses_weights[index_bev] = 0.0

    # Create model and optimizers
    '''
        a custom model designed for Lidar-based object detection or center point estimation.
    '''
    ...
    if ((bool(args.zero_redundancy_optimizer) == True) and (parallel == True)):
        '''
            该优化器专为分布式训练场景而设计，可在 DistributedDataParallel (DDP) 训练期间节省 GPU 内存。
        '''
        ...
        # This is the optimizer used for single GPU training.
        optimizer = optim.AdamW(model.parameters(), lr=args.lr) # For single GPU training

    '''
        filtering the model.parameters() to include only the parameters that require gradients. 
        This ensures that only the trainable parameters are considered for counting.

        The code provides a way to examine and report the number of trainable parameters in the model. 
        It can be helpful for understanding the model's complexity, memory requirements, 
        and estimating the computational resources needed during training.
    '''
    model_parameters = filter(lambda p: p.requires_grad, model.parameters())
    # accumulates the total count of trainable parameters in the model.
    # p.size(): (3, 4, 5)
    # np.prod(p.size()) : 3 * 4 * 5 = 60
    ...
    # The generator is used to generate random numbers and 
    # provides control over the randomness in PyTorch operations.
    '''
        将生成器的设备设置为“cpu”意味着它将在 CPU 而不是 GPU 上生成随机数。
        当您想要确保在不同设备上一致地生成随机数时，或者当您需要为仅 CPU 操作生成随机数时，这很有用。

        通过将手动种子设置为全局随机数生成器的初始种子，您可以在每次运行代码时获得相同的随机数序列，
        只要适当控制其他随机源（例如，不依赖于其他随机操作）系统时间等外部因素）
    '''
    ...
    if(parallel == True):
        '''
            The DistributedSampler is designed to be used in distributed training scenarios where multiple processes work together 
            to train a model. It ensures that each process operates on a unique subset of the data by dividing the dataset 
            into smaller shards and assigning each process a specific shard to work on.

            By using the DistributedSampler with the training dataset, each process will iterate over a 
            different portion of the dataset during training, enabling efficient and synchronized training across multiple processes. 
            The shuffling of data ensures that the order in which data is presented to each process is different in each epoch, 
            further enhancing the randomness and preventing any potential biases.
        '''
    ...
    # Create logdir
    # isdir: determine if the path corresponds to an existing directory.
    ...
    if(rank == 0):
        ...
        with open(os.path.join(args.logdir, 'args.txt'), 'w') as f:
            # writes the content of args.__dict__ to the file in JSON format with an indentation level of 2. 
            # The args.__dict__ attribute returns a dictionary containing all the arguments as key-value pairs.
    ...
    if (not (args.load_file is None)):
        # Load checkpoint
        print("=============load=================")
        # map_location parameter is set to model.device to ensure that the loaded model parameters are mapped to the correct device.
        model.load_state_dict(torch.load(args.load_file, map_location=model.device))
        # This line loads the state dictionary of the optimizer from a separate checkpoint file
    ...
    for epoch in range(trainer.cur_epoch, args.epochs):
        ...
        '''
            This condition ensures that the learning rate reduction is applied only 
            at the specified epochs and when the schedule option is set to 1.

            this code snippet implements a learning rate schedule where the learning 
            rate is reduced by a factor of 10 at specific epochs during training.
        '''
        ...
        # This condition is used to determine whether to perform validation during training.
        ...
        if (parallel == True):
            if (bool(args.zero_redundancy_optimizer) == True):
                '''
                    The argument 0 specifies that the consolidated state should be gathered on GPU 0. 
                    This step is required to save the complete optimizer state when using the zero redundancy optimizer.
                '''
...
class Engine(object):
    ...
    def __init__(self, model, optimizer, dataloader_train, dataloader_val, args, config, writer, device, rank=0, world_size=1, parallel=False, cur_epoch=0):
        ...
        # enumerate() function, which provides both the index (idx) and the value (key)
        # creates a dictionary self.detailed_weights where 
        # the keys: are taken from self.detailed_losses 
        # the values: are taken from detailed_losses_weights
    ...
    def load_data_compute_loss(self, data):
        ...
        if self.config.multitask:
            ...
            # squeeze(1): 从张量中删除索引 1 处的单例维度。这通常在处理具有额外维度的张量时使用，但它代表一个单一维度并且不需要。
            # The torch.long data type is commonly used for integer values
    ...
    '''
        iterates over the training data, computes the losses, performs backpropagation, and updates the model parameters. 
        It also keeps track of the total loss and detailed losses for each epoch and logs the relevant information.
    '''
    def train(self):
        self.model.train()

        # tracks the number of batches processed during the training loop
        num_batches = 0
        # accumulates the total loss across all batches for the current epoch.
        loss_epoch = 0.0
        # This dictionary initializes the detailed losses for the current epoch to 0.0
        detailed_losses_epoch  = {key: 0.0 for key in self.detailed_losses}
        self.cur_epoch += 1

        # Train loop
        for data in tqdm(self.dataloader_train):
            # This line clears the gradients of all model parameters. 
            # It sets the gradients to None instead of 0 to optimize memory usage.
            self.optimizer.zero_grad(set_to_none=True)
            # loads the data and computes the losses for the current batch
            losses = self.load_data_compute_loss(data)
            # initializes the total loss for the current batch to 0.0. 
            # It creates a tensor with value 0.0 and moves it to the device specified by self.device.
            loss = torch.tensor(0.0).to(self.device, dtype=torch.float32)

            # The code enters a nested loop to calculate the weighted sum of losses for each detailed loss.
            for key, value in losses.items():
                # calculates the weighted loss for each detailed loss. 
                # It multiplies the value of the loss by the corresponding weight
                loss += self.detailed_weights[key] * value
                detailed_losses_epoch[key] += float(self.detailed_weights[key] * value.item())
            
            # This line performs backpropagation by 
            # computing the gradients of the loss with respect to the model parameters.
            loss.backward()
            
            # updates the model parameters 
            # by taking a step in the optimizer's direction using the computed gradients
            self.optimizer.step()
            # increments the num_batches counter by 1 for each processed batch.
            num_batches += 1
    ...
    '''
        Inference mode is typically used when running inference or evaluation on a trained model, 
        where no gradient computations or backpropagation are needed.
    '''
...
'''
    通过为 NumPy 和随机模块设置种子特定于工作程序的种子，有助于确保工作进程内的随机数生成在多次运行中保持一致。
    这对于运行涉及随机元素的代码（例如数据混洗或某些组件的初始化）时的可重复性很有用。
'''
...
if __name__ == "__main__":
    ...
    # 通过将启动方法设置为“fork”，代码表明它希望在使用 torch.multiprocessing 时使用“fork”方法创建子进程。
    # 这可确保使用“fork”启动方法创建子进程，这有利于提高性能和内存效率。
